...
- 处理器接口：
  - IPermissionHandler：处理 session/request_permission 请求。
  - IFileSystemHandler：处理 fs/read_text_file 与 fs/write_text_file。
  - ITerminalHandler：处理 terminal/* 系列请求。
- 协议与传输：
...
### 处理器接口与扩展点
- IPermissionHandler：处理权限请求，需阻塞直到用户做出选择，返回 RequestPermissionResponse。
- IFileSystemHandler：读取/写入文本文件，供 Agent 访问文件系统。
- ITerminalHandler：创建终端、获取输出、等待退出、终止与释放资源。
