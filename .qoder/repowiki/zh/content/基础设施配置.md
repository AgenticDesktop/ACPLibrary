...
## 更新摘要
**所做更改**   
- 更新了依赖注入容器配置和服务注册机制的详细文档
- 增强了 JSON 序列化选项的配置说明和行为描述
- 完善了服务生命周期管理的解释
- 添加了完整的配置示例和最佳实践指南
- 改进了性能调优和内存管理建议
...
ServiceCollectionExtensions 提供了 AddAcpClient 扩展方法，用于将 ACP 客户端相关服务注册到依赖注入容器中。该方法具有以下特点：
...
- **生命周期管理**：
  - **单例模式**：AcpClient 和 IAcpClient 在整个应用生命周期内保持唯一实例，确保资源的有效管理和状态的一致性
  - **瞬态模式**：JsonRpcDispatcher 和 RequestTracker 每次解析时创建新实例，适用于无状态或轻量对象

- **使用方式**：在应用启动时调用 `services.AddAcpClient()` 完成所有服务的自动注册

**章节来源**
- [Infrastructure/ServiceCollectionExtensions.cs:11-21](file://Infrastructure/ServiceCollectionExtensions.cs#L11-L21)
...
StdioAgentTransport 基于子进程的标准输入输出实现 JSON-RPC 通信：
...
- **内存管理策略**：
  - AcpClient 实现 IAsyncDisposable，确保资源正确释放
