...
## 简介
本文件系统性阐述该库对 JSON-RPC 2.0 协议的完整实现，覆盖消息模型、序列化/反序列化机制、分发与传输集成、字段校验与错误处理，以及版本兼容性与扩展性设计。通过详细的英文文档注释，开发者可以深入了解每种消息类型的属性、结构和用途，掌握 JsonRpcMessageConverter 的序列化和反序列化逻辑，包括自定义转换器配置和兼容性处理。

## 项目结构
JSON-RPC 相关代码主要分布在以下模块：
- **JsonRpc**：协议消息类型与自定义转换器，包含完整的XML文档注释
...
graph TB
subgraph "JsonRpc"
M["JsonRpcMessage<br/>基类，固定jsonrpc字段为'2.0'"]
Rq["JsonRpcRequest<br/>请求消息，包含id、method、可选params"]
Rs["JsonRpcResponse<br/>响应消息，包含id，result与error二选一"]
Nt["JsonRpcNotification<br/>通知消息，包含method与可选params，无id"]
Er["JsonRpcError<br/>错误对象，包含code、message、可选data"]
Cvt["JsonRpcMessageConverter<br/>根据字段存在性自动识别消息类型"]
end
subgraph "Infrastructure"
Opt["JsonOptions<br/>全局JsonSerializerOptions"]
end
subgraph "Protocol"
Disp["JsonRpcDispatcher<br/>负责发送请求/通知、接收消息、路由到处理器"]
IfDisp["IJsonRpcDispatcher<br/>分发器接口"]
RT["RequestTracker<br/>请求跟踪器"]
end
subgraph "Transport"
TrIf["IAgentTransport<br/>传输层抽象"]
Stdio["StdioAgentTransport<br/>stdio实现"]
end
M --> Rq
M --> Rs
M --> Nt
...
- [JsonRpcMessageConverter.cs:6-32](file://JsonRpc/JsonRpcMessageConverter.cs#L6-L32)
...
## 核心组件
- **JsonRpcMessage**：JSON-RPC 2.0 基类，固定 jsonrpc 字段为 "2.0"，用于区分消息类型的基础标记。作为所有 JSON-RPC 2.0 消息的基类，确保版本一致性。
- **JsonRpcRequest**：请求消息，包含 id（long）、method（string）、可选 params（JsonElement?）。发起一次带标识的请求，服务端需以相同 id 返回结果或错误。
- **JsonRpcResponse**：响应消息，包含 id（long），且 result（JsonElement?）与 error（JsonRpcError?）二选一。对应请求的结果；result 与 error 互斥。
- **JsonRpcNotification**：通知消息，包含 method（string）与可选 params（JsonElement?），无 id。单向通知，无需响应，不包含 id。
- **JsonRpcError**：错误对象，包含 code（int）、message（string）、可选 data（JsonElement?）。标准化错误信息，便于上层统一处理。
- **JsonRpcMessageConverter**：根据字段存在性自动识别消息类型并进行序列化和反序列化。转换器具有递归防护和兼容性支持。
- **JsonOptions**：全局 JsonSerializerOptions，注册上述转换器与枚举字符串化策略。提供统一的序列化配置。
- **JsonRpcDispatcher**：负责发送请求/通知、接收消息、路由到处理器、关联请求与响应。实现 IJsonRpcDispatcher 接口。
- **RequestTracker**：管理待处理的请求，分配唯一ID并跟踪任务完成状态。
- **IAgentTransport / StdioAgentTransport**：抽象与 stdio 实现，提供行式 JSON 消息的收发。支持进程级通信。
...
- **JsonRpcMessage**
  ...
  - 文档注释：JSON-RPC 2.0 base message type. Distinguishes request/response/notification by field presence.
- **JsonRpcRequest**
  - 属性：id（long）、method（string）、params（可选，JsonElement?）
  - 用途：发起一次带标识的请求，服务端需以相同 id 返回结果或错误
  - 文档注释：JSON-RPC 2.0 request (has method + id)
- **JsonRpcResponse**
  - 属性：id（long）、result（可选，JsonElement?）、error（可选，JsonRpcError?）
  - 用途：对应请求的结果；result 与 error 互斥
  - 文档注释：JSON-RPC 2.0 response (has id + result or error)
- **JsonRpcNotification**
  - 属性：method（string）、params（可选，JsonElement?）
  - 用途：单向通知，无需响应，不包含 id
  - 文档注释：JSON-RPC 2.0 notification (has method, no id)
- **JsonRpcError**
  - 属性：code（int）、message（string）、data（可选，JsonElement?）
  - 用途：标准化错误信息，便于上层统一处理
  - 文档注释：JSON-RPC 2.0 error object
...
**章节来源**
- [JsonRpcMessageConverter.cs:6-71](file://JsonRpc/JsonRpcMessageConverter.cs#L6-L71)
- [JsonOptions.cs:13-26](file://Infrastructure/JsonOptions.cs#L13-L26)
...
## 结论
该库对 JSON-RPC 2.0 的实现清晰、可扩展且注重性能。通过统一的基类与派生类型、智能的转换器与全局序列化选项、健壮的分发与请求跟踪机制，以及与 stdio 传输的解耦设计，能够稳定支撑 ACP 场景下的双向通信与通知流。详细的英文文档注释使开发者能够更好地理解每种组件的职责和使用方式。建议在扩展新方法与通知时遵循现有模式，保持字段命名与可选性一致，以确保兼容性。
